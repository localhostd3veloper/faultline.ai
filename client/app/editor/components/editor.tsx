...
import { FileCode, FileJson, Trash2, WandSparkles } from "lucide-react";
                ...
                <div className="h-full overflow-auto rounded-lg [&_textarea]:h-full [&_textarea]:resize-none [&_textarea]:outline-none [&_textarea]:focus:outline-none [&_textarea]:focus-visible:outline-none">
                  <CodeEditor
                    ...
                    highlight={(code: string) => {
                      if (!isHighlighterReady) return code;
                      const lang = languages.markdown;
                      if (!lang) return code;
                      return highlight(code, lang, "markdown");
                    }}
                    padding={12}
                    style={{
          ...
          {isFetched && !showSource ? (
            <div className="bg-background flex flex-1 flex-col items-center justify-center rounded-lg border border-dashed">
              <div className="flex flex-col items-center gap-4 text-center">
                ...
                <div>
                  <h4 className="text-lg font-medium">OpenAPI Schema Loaded</h4>
                ...
                <CodeEditor
                  ...
                  highlight={(code: string) => {
                    ...
                    if (contentType === "openapi-json") {
                      const lang = languages.json;
                      if (!lang) return code;
                      return highlight(code, lang, "json");
                    } else if (contentType === "openapi-yaml") {
                      const lang = languages.yaml;
                      if (!lang) return code;
                      return highlight(code, lang, "yaml");
                    }
                    const lang = languages.markdown;
                    if (!lang) return code;
                    return highlight(code, lang, "markdown");
